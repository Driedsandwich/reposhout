...
**いったん作り直しています。** 第12回監査 R12-001（資格情報の検出漏れ）と R12-002
（普通の検索を落としていた）のため、先に作った成果物
（`reposhout-package-cf04a54…` / SHA-256 `b4bb967e…`）は**提出しません**。
却下した成果物は正本の `history` に理由つきで残しています。
