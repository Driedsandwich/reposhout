...
**いま出せる成果物はありません。** 第16回監査で直前の候補（`reposhout-package-d75b2bfe…` /
35,798 B / SHA-256 `97bcf769…`）を `rejected_by_R16` として外しました。R16の修正が main に
...
**「最新の main」では選ばず、正本に書いた成果物名とSHA-256 で選んでください**（第10回監査 R10-006）。
