...
**② User activity**: `x.com` 側で `keydown` を1つ監視していますが、見ているのは Escape かどうかだけで、保存も送信もしません（拡張が自分で開いた投稿画面を閉じるためです）。この欄は「操作の監視（クリック・マウス位置・スクロール・キーロギング等）」を指すので **No と判断**しましたが、コードの事実として上に書いておきます。ダッシュボードの現行の説明文を読んで、最終的には本人が決めてください。
