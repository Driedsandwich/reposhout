...
The X script is deliberately blind to page content. Before closing anything it asks the service worker whether this window is one the extension itself opened, and the only evidence accepted is the window ID recorded at creation time. **Opening a window and being able to close it with Escape are two different facts, and the extension keeps them apart**: if the browser does not return a window ID, or the record cannot be written, the window is not treated as one of its own — Escape will not close it, and the extension tells you once rather than reporting a plain success. <!-- HISTORICAL_CLAIM:start reason="1.1.7までの挙動。現在の仕様ではない" -->Until 1.1.7 it said so only when the browser returned no window ID; when the record could not be written it stayed silent.<!-- HISTORICAL_CLAIM:end --> **If it does not match — which is the case for every X tab you opened yourself — it does nothing.** It cannot close your normal X tabs. The same applies when a popup could not be created and the composer opened in an ordinary tab: that tab is not recorded as the extension's own, so Escape leaves it alone.
...
X側のスクリプトは、意図的にページの中身を見ません。閉じる前に、service worker へ「このウィンドウは拡張が開いたものか」とだけ尋ねます。根拠として使うのは、開いた時点で記録したウィンドウIDだけです。**「開いた」ことと「Escで閉じられる」ことは別の事実として扱います**——ブラウザがウィンドウIDを返さなかったときや、記録を書けなかったときは、自分の窓として扱いません（Escでは閉じず、成功と報告もしません）。**一致しない場合（＝あなたが自分で開いたXのタブはすべてこれに当たります）、何もしません。** 通常のXのタブを閉じることはできません。
