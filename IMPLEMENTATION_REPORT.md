...
| 1.1.3 | 4回目の監査への対応。**作業ツリーにあるだけで、commit も push もしていない** |
| ストア | **公開中は 1.0.1 のまま。1.1.0 も 1.1.1 も提出していない** |
| 監査 | 2026-08-05 に外部レビューが4回。**指摘は 4件 → 7件 → 5件 → 6件。4回目の1件（R4-001）だけは再現せず、指摘の前提が事実と違った** |
| ストア提出 | **外部監査に合格してから出す**（本人決定・2026-08-05）。判定は4回とも NOT_READY |
