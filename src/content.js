 ...
 *
 * 設計方針（壊れにくさを最優先）:
 *  1. 依存する目印は3つだけ。見つからなければ**ボタンを置かない**
 *     （⚠️「何もしない」ではない。拒否の案内はこの経路と無関係に出る＝下の 3）
 *  2. GitHubの既存DOMは読むだけ。書き換え・削除は一切しない
 *  3. 足す要素は次の3種類だけ（第22回監査 R22-001 で数え直した）:
 *       ・head へ <style> 1つ
 *       ・操作列へボタンの入れ物1つ（見つかったときだけ）
 *       ・body へ拒否の案内 <div id="gxs-notice"> 1つ
 *         **操作列が無いページでも足す**——ツールバーとショートカットは
 *         そこでも動くので、断った理由を伝える先が要る
 *  4. ボタンの個数・種類（Pin/Watch/Notifications等）は一切見ない。
 *     見ないことでログイン/ログアウトの差と将来の増減を吸収する
 *  5. 置くために読むのは決まった範囲だけ——目印の有無、先頭の子の作り
 *     （children / tagName）、配置合わせの表示値（display / cssFloat /
 *     marginRight）、隣のボタンの高さ。ページの本文も入力欄も読まない。
 *     タイトルは読まない。URLもここでは読まない（見るのは service worker）
 */
(function () {
  'use strict';
  ...
  /* ------------------------------------------------------------
   * 画面へ短い案内を出す（値は出さない）。用途は2つ（第25回監査 R25-004）:
   *   ① 開かなかった理由を伝える（第12回監査 R12-002）
   *   ② **開いたが Esc では閉じられない**ことを伝える（第24回監査 R24-003）
   * ②は共有そのものは成功しているので「送っていない」とは書かない。
   * ------------------------------------------------------------
