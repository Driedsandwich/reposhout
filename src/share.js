...
(function (root) {
   ...
   *
   * 第12回監査 R12-001。1.1.7 までは名前の allowlist だけで、値は
   ...
   *   enum  決めた語のどれか
   *
   * **残せる型はこの3つだけ**（第13回監査 R13-001 で slug を廃止した。理由は
   * valueFitsRule の直前に書いてある）。型を足すときは、その型が取りうる値を
   * 数え切れることを先に示すこと。
   *
   ...
   * ============================================================
   *
   * HISTORICAL_CLAIM:start reason="第14回までの設計。現在の仕様ではない"
   ...
   *  - **フラグメント（#…）は種類を問わず、すべて落とす**（第15回監査 R15-001）。
   *    `#L10-L20`・`#issuecomment-123` も残さない。任意の文字列を運べるため
   *  - 資格情報の形をしたハッシュは、落とす前に検査もする（2層目）
   *
   ...
   * **これを呼ぶのは service worker だけ**（src/background.js）。画面側の
   * content script は share.js を読み込まない（第16回監査 R16-003）。
  ...
  /*
   * 本体。**URLだけ**から投稿用の文面とXのURLを作る（引数は1つ）。
   * 文面はルート（Issue / PR / Commit など）から生成する。
   * ページのタイトルは読みも受け取りもしない（第15回監査 R15-001）。
   * github.com 以外なら ok:false。
   *
   * 注: Open / Merged / Closed の状態ラベルは意図的に含めない。
   * ログイン状態とログアウト状態で取得値が食い違う（同一PRが Merged / Open）
   * 事象を実測しており、誤った状態を投稿するリスクを避けるため。
   *
   * 理由つきの入口。呼び出し側（= service worker だけ）は reason を見て、
   * 値を含まない定型の案内を出す（第12回監査 R12-002）。
   */
