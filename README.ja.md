...
- **文字数は X の公表している規則に沿わせ、ずれるときは必ず多めに数える。** 既定の重みは2で、重み1は4つの範囲だけ。絵文字は書記素クラスタ1つで2、ドメインらしきトークンは長さによらず23、先にNFC正規化します。**公式の `twitter-text` パーサーそのものではなく、公式のテストコーパスも走らせていません**。ずれる場合は必ず多め（＝早めに切り詰める）側へ倒しています。少なく数える方向だけが「Xに弾かれる文面を作る」事故につながるためです
...
`storage` は 1.1.0 で追加しました。入るのはウィンドウIDと開いた時刻だけで、URLもページ内容も履歴も入りません。どこにも送信しません。
...
拡張は独自の通信を一切行いません。保存するのは上記のウィンドウIDだけで、ブラウザを終了すると消えます。詳細は [PRIVACY.md](PRIVACY.md) を参照してください。
...
| 種類 | コマンド | 内容 |
|---|---|---|
| 単体・適合 | `npm run test:unit` | 手書きの期待値103件に対する35テスト。twitter-text v3 の定義に対する文字重み、ページ種別ごとのURL方針、タイトル整形、絵文字301位置での切り詰め安全性 |
...
2026-08-05 実測: 単体35件 PASS / 0 FAIL、E2E 10件 PASS / 0 FAIL、ブラウザ版 195項目 / 0 FAIL。Escの2件は 1.0.1 の実装に戻すと落ちることも確かめています（落ちないテストは何も証明しません）。
