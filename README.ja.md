...
やっぱりやめたくなったら、共有ウィンドウで <kbd>Esc</kbd> を押すと閉じます——**効くのは、拡張が開いてウィンドウIDを記録できたポップアップ窓だけ**です。記録できなかったときは、その場で1回だけお知らせしますので、ご自身で閉じてください。ポップアップを作れずタブで開いた場合、そのタブは拡張の持ち物として扱いません（Escの対象外です）。ふつうのタブと同じように閉じてください。
...
**これは「拒否する名前の一覧」であって、「その名前が実在の所有者だ」と示すものではありません。** GitHubが新しい機能ページを増やせば、その名前は一覧を直すまでリポジトリとして扱われます。一覧へ足してよいのは次の**3つ**で、語ごとにどれを使ったかを `denyCriterionId` として [`store/GITHUB_NAMESPACE_INVENTORY.json`](store/GITHUB_NAMESPACE_INVENTORY.json) に記録してあります。**①その名前のアカウントが無い**（そこにリポジトリを作れないので、拒否しても実在のリポジトリを巻き込まない・68語）。**②アカウントは在るが、ブラウザでリポジトリの画面が出ない**——GitHub自身の案内ページが名前空間を覆っていて、そこのリポジトリは開くことすらできません（`customer-stories` がこれで、アカウントは在り公開リポジトリも1件ありますが、ブラウザで開くと404になります・1語）。**③アカウントは在るが公開リポジトリが0件で、認証・アカウント系の入口である**（`watching`・`authorize`・`verify`・3語）。開けるリポジトリを持つ名前をそれでも拒否する4つ目の枠は**予約だけで現在0件**です。使うときは、何を巻き込むかを必ず書き残します。**アカウントが在って、しかもリポジトリが開ける**場合は足しません——`skills`・`accessibility`・`security-lab` は機能ページのように見えますが、公開リポジトリを持つ実在の組織なので**わざと足していません**。<!-- HISTORICAL_CLAIM:start reason="第22回までのREADMEの説明。実際の採用理由と食い違っていた" -->（第23回監査まで、ここには「アカウントが無いことを確かめたものだけ」と書いていましたが、4語がそれに当てはまりませんでした。）<!-- HISTORICAL_CLAIM:end -->
...
X側のスクリプトはページの中身を一切読みません。**「開いた」ことと「Escで閉じられる」ことは別の事実です**——ブラウザがウィンドウIDを返さなかったときや、記録を書けなかったときは、拡張自身の窓として扱わないので、Escでは閉じません。**そのことは、3つの入口（ツールバー・ショートカット・画面内ボタン）のどれから開いても、その場で1回だけお知らせします**——効かない閉じ方を探して詰まらないようにするためです。ウィンドウを閉じる前に、service workerへ「これはあなたが開いた窓か」とだけ尋ねます。答えの根拠は `chrome.windows.create()` が返したウィンドウIDだけで、これはページ側から読むことも詐称することもできません。
