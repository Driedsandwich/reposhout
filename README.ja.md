...
- **クエリの値が資格情報の形をしているページは共有しません。** 1.1.8 から、共有URLに残る値に `access_token=`・`client_secret:`・JSONのトークン欄・`Bearer` の値・GitHubのトークンの接頭辞が入っていれば、そのパラメータだけを黙って消すのではなく**URLごと拒否**します。語をただ検索しているだけ（`how to use access_token` など）は、これまでどおり共有できます。
