...
- **残ったURLが資格情報の形をしている場合は、URLごと拒否します。** 例: `.../blob/main/access_token=…` のようなパス、GitHub・`Bearer`・AWS・Slack・JWT・秘密鍵の形。一部だけを黙って消すのではなく全体を断り、**値もURLも出さない短い案内**を表示します。**見ているのはここに挙げた形だけで、「あらゆる秘密を検出できる」という意味ではありません。** 語をただ含むだけ（`access_token.md` というファイル名など）は、これまでどおり共有できます。
