...
**これは「拒否する名前の一覧」であって、「その名前が実在の所有者だ」と示すものではありません。** GitHubが新しい機能ページを増やせば、その名前は一覧を直すまでリポジトリとして扱われます。一覧へ足してよいのは次の2つで、語ごとの理由は [`store/GITHUB_NAMESPACE_INVENTORY.json`](store/GITHUB_NAMESPACE_INVENTORY.json) に記録してあります。**①その名前のアカウントが無い**（そこにリポジトリを作れないので、拒否しても実在のリポジトリを巻き込まない）。**②アカウントは在るが、ブラウザでリポジトリの画面が出ない**——GitHub自身の案内ページが名前空間を覆っていて、そこのリポジトリは開くことすらできません。`customer-stories` が②で、アカウントは在り公開リポジトリも1件ありますが、ブラウザで開くと404になります。**アカウントが在って、しかもリポジトリが開ける**場合は足しません——`skills`・`accessibility`・`security-lab` は機能ページのように見えますが、公開リポジトリを持つ実在の組織なので**わざと足していません**。<!-- HISTORICAL_CLAIM:start reason="第22回までのREADMEの説明。実際の採用理由と食い違っていた" -->（第23回監査まで、ここには「アカウントが無いことを確かめたものだけ」と書いていましたが、4語がそれに当てはまりませんでした。）<!-- HISTORICAL_CLAIM:end -->
