...
- **自由に書ける欄は共有しません。** 検索語（`q`・`query`・`discussions_q`）と、作成中のプルリクエストの `title`・`body` は共有URLから落とします。残すのは、値の形を機械的に確かめられるもの（ページ番号・状態・並び順・ラベルなど）だけです。**有限の一覧では「自由文に秘密が入っていない」ことを示せない**ので、そもそも運ばない方針にしています。
