...
X側のスクリプトはページの中身を一切読みません。**「開いた」ことと「Escで閉じられる」ことは別の事実です**——ブラウザがウィンドウIDを返さなかったときや、記録を書けなかったときは、拡張自身の窓として扱わないので、Escでは閉じません。ウィンドウを閉じる前に、service workerへ「これはあなたが開いた窓か」とだけ尋ねます。答えの根拠は `chrome.windows.create()` が返したウィンドウIDだけで、これはページ側から読むことも詐称することもできません。
