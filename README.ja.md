...
| サイト | 目的 |
|---|---|
| `github.com` | Share ボタンの設置。ページを読むのはボタン行を探すためだけで、`<style>` を1つとボタン1個を包む要素を1つ足す以外は何も変えません。 |
...
- **自由に書ける欄は共有しません。** 検索語（`q`・`query`・`discussions_q`）と、作成中のプルリクエストの `title`・`body` は共有URLから落とします。残すのは、値の集合を数えられるもの（ページ番号・状態・並び順・真偽値）だけです。`labels`・`author`・`branch`・`path` のような識別子っぽい値も落とします（1.1.8）——どれにもトークンを入れられるからです。**有限の一覧では「自由文に秘密が入っていない」ことを示せない**ので、そもそも運ばない方針にしています。
- **残ったURLが資格情報の形をしている場合は、URLごと拒否します。** 例: `.../blob/main/access_token=…` のようなパス、GitHub・`Bearer`・AWS・Slack・JWT・秘密鍵の形。一部だけを黙って消すのではなく全体を断り、**値もURLも出さない短い案内**を表示します。同じ検査は投稿本文と、**変換する前の生のタイトル**にも掛けます（1.1.8）——長いタイトルを切り詰めるとトークンが短くなり、パターンの最小長を下回って断片がXへ渡っていたためです。照合は素のままと、見えない文字を落として正規化（NFKC）した形の両方で行うので、全角の `access_token＝…` や、ゼロ幅スペースで割ったトークンも当たります。**見ているのはここに挙げた形だけで、「あらゆる秘密を検出できる」という意味ではありません。** 語をただ含むだけ（`access_token.md` というファイル名など）は、これまでどおり共有できます。
