...
Chromeの `activeTab` の設計上、これは利用者が拡張を明示的に操作した瞬間（ツールバー押下・ショートカット）に、そのタブに対してのみ付与されます。バックグラウンドでの閲覧監視はできません。

コンテンツスクリプトは `https://github.com/*` に限定されます。拡張は独自の通信を行わず、データも保存しません。詳細は [PRIVACY.md](PRIVACY.md) を参照してください。
...
`test/share.test.html` をブラウザで開くと実行されます。2026-08-01時点で **104ケース全PASS**。

絵文字を切り詰め位置に1文字ずつずらして当てる網羅テスト71件、CJKの重み付き文字数、GitHubの予約名前空間などを含みます。
