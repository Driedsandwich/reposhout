...
Until 1.1.8 the extension sent `document.title` and the raw path and relied on a detector to refuse anything shaped like a credential. Measured against that build, the approach failed from both sides at once: 37 realistic credential formats reached the composer (`X-Api-Key:`, `AWS_SECRET_ACCESS_KEY=`, `SAMLResponse=`, `ya29.…`, `hf_…` among them), while four ordinary developer titles — including *How to parse key=value pairs* — were wrongly refused. Tightening the detector broke the product and loosening it leaked, so the boundary is now structural. The credential detector remains as a second layer, not as the boundary.
