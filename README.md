...
- **Pages whose query values look like credentials are refused.** From 1.1.8, if a value that would remain in the shared URL contains something shaped like a credential (`access_token=`, `client_secret:`, a JSON token field, a `Bearer` value, a GitHub token prefix), the whole URL is refused rather than the parameter silently dropped. Merely mentioning the word — searching for `how to use access_token` — still shares.
