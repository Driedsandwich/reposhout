...
- **Free-text query values are never shared.** Search terms (`q`, `query`, `discussions_q`) and prepared pull-request `title` / `body` are dropped from the shared URL. Only values whose shape can be checked mechanically are kept — page numbers, states, sort orders, labels and similar. This is deliberate: no finite list of patterns can prove that arbitrary text contains no secret.
- **Pages whose remaining URL looks like it carries a credential are refused outright** — for example a file path such as `.../blob/main/access_token=…`, or a GitHub / `Bearer` / AWS / Slack / JWT / private-key token shape. The whole URL is refused rather than a part of it silently removed, and a short notice says so without showing the URL or the value. **The check covers the patterns listed above; it is not a guarantee that every possible secret is detected.** Merely mentioning the word — a file named `access_token.md` — still shares.
