...
**This is a list of names to refuse, not proof that a name is a real owner** — if GitHub adds a new function page, that name will be treated as a repository until the list is updated. A name may be added under one of two conditions, and the reason for each name is recorded in [`store/GITHUB_NAMESPACE_INVENTORY.json`](store/GITHUB_NAMESPACE_INVENTORY.json): either **no account holds the name** (nothing can be built there, so refusing it takes nothing real with it), or **an account exists but the browser does not render a repository UI under it** — GitHub's own page shadows the namespace, so a repository there cannot be opened, let alone shared. `customer-stories` is the second kind: the account exists and even has one public repository, yet opening it in a browser returns 404. When an account exists *and* its repositories do open, the name is left alone: `skills`, `accessibility` and `security-lab` all look like function pages but are genuine organisations with public repositories, and they are deliberately not on the list. <!-- HISTORICAL_CLAIM:start reason="第22回までのREADMEの説明。実際の採用理由と食い違っていた" -->(Until the twenty-third audit this paragraph said a name was added only when no account held it, which did not match four of the entries.)<!-- HISTORICAL_CLAIM:end -->
